...
static int wait_for_http_get_ok(const char *url)
{
...
int main(void)
{
    ...
    if (!wait_for_http_get_ok(capture_health) || !wait_for_http_get_ok(worker_health) || !wait_for_http_get_ok(clickhouse_url)) {
        curl_global_cleanup();
        return 1;
    }
