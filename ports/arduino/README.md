...
Preview `0.3.1`. Use for evaluation or controlled pilots until your product has passed hardware, TLS, offline queue, flush, retry, and power-cycle validation on the target ESP32 board.
...
Crash reporting is automatic — there is no manual error method. When
`enableErrorTracking` is set, the wrapper maps the ESP32 reset reason into a
one-time `$crash` event emitted during `begin()` after an abnormal reset.
