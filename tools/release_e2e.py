...
class Step:
    ...
    require_python_modules: tuple[str, ...] = ()
    optional: bool = False
...
STEPS: tuple[Step, ...] = (
    ...
    step(
        "Local capture",
        "record platform capture contract when available",
        "if [ -d ../platform/capture ]; then "
        "git -C ../platform branch --show-current; "
        "git -C ../platform log -1 --oneline; "
        "git -C ../platform log -1 --oneline -- capture; "
        "rg -n '\"/capture\"|\"/e\"|\"/chunks\"|application/vnd.honch.chunk|X-Honch-Project-Key|X-Honch-Stream-Id|POST /batch|application/cbor' "
        "../platform/capture/README.md ../platform/capture/src ../platform/capture/tests || true; "
        "else echo '../platform/capture not present'; fi",
        SERVICES,
    ...
    step(
        ...
        "prefix=\"micropython_e2e_$(date +%s)_$$\"; "
        "HONCH_MICROPYTHON_E2E_PREFIX=\"$prefix\" "
        "MICROPYPATH=\"ports/micropython:"
        + MICROPYTHON_UREQUESTS_PATH
        + ":"
        + MICROPYTHON_REQUESTS_PATH
        + "\" "
        "\"${MICROPYTHON_BIN:-" + MICROPYTHON_UNIX_BIN + "}\" ports/micropython/tests/runtime_e2e_capture.py && "
        "HONCH_MICROPYTHON_E2E_PREFIX=\"$prefix\" "
        "python3 ports/micropython/tests/verify_e2e_capture.py",
        SERVICES,
        "24-micropython-e2e-run.txt",
        env=DEFAULT_E2E_ENV,
        require_commands=("python3",),
        require_paths=(MICROPYTHON_UNIX_BIN, MICROPYTHON_UREQUESTS_PATH, MICROPYTHON_REQUESTS_PATH),
    ),
    step(
        "Local capture",
    ...
    step(
        "ESP-IDF",
        "verify ESP-IDF QEMU toolchain",
        f"{ESP_IDF_SOURCE} && qemu-system-xtensa --version && "
        "/Users/morgana/Development/honch-io/sandbox/honch --plain sandbox qemu doctor",
        TOOLCHAINS,
        "34-esp-qemu-doctor.txt",
        require_commands=("qemu-system-xtensa",),
        require_paths=(ESP_IDF_TOOLS, ESP_IDF_EXPORT, QEMU_XTENSA_BIN_DIR),
    ),
    step(
        "Arduino ESP32",
    ...
    step(
        ...
        f"PLATFORMIO_CORE_DIR='{PLATFORMIO_CORE_DIR}' pio run -d ports/arduino/examples/platformio",
        TOOLCHAINS,
        "64-arduino-platformio.txt",
        require_commands=("pio",),
        require_paths=(PLATFORMIO_CORE_DIR,),
    ),
    step(
        "MicroPython",
    ...
    step(
        ...
        "cd mobile/react-native-relay && bun run typecheck && bun run test",
        ("host", "release", "full"),
    ...
    step(
        ...
        "mobile/react-native-relay spec/relay-envelope.md spec/relay-chunks.md || true",
        ("host", "release", "full"),
    ...
    step(
        ...
        "core mobile/react-native-relay spec/relay-chunks.md",
        HOST,
...
def missing_requirements(entry: Step, env: dict[str, str]) -> list[str]:
    ...
    for module in entry.require_python_modules:
        ...
        if completed.returncode != 0:
            missing.append(f"python module not found: {module}")
    return missing
...
def run_step(entry: Step, log_dir: Path, dry_run: bool) -> Result:
    ...
    env.setdefault("PLATFORMIO_CORE_DIR", PLATFORMIO_CORE_DIR)
